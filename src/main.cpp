...
static bool sRememberDebug = false;
static BetterSMS::Settings::BoolSetting sRememberDebugSetting("Start in Debug Mode", &sRememberDebug);

/*
/ Module Info
*/

static BetterSMS::ModuleInfo sModuleInfo("Better Debug Entry", 1, 1, &sSettingsGroup);

/*
/ Callbacks
*/

BETTER_SMS_FOR_CALLBACK static void autoDebug(TApplication *tapp) {
    BetterSMS::setDebugMode(sRememberDebug);

    OSReport("\n\n---Auto Setting Debug Mode!--- :3\n\n");
}

BETTER_SMS_FOR_CALLBACK static void removeSetting(TMarDirector *dir) {

    bool settingRemoved = false;

    if (!(BetterSMS::isDebugMode()) && !settingRemoved) {
        sSettingsGroup.removeSetting(&sRememberDebugSetting);
        settingRemoved = true;
    }
}
...
static void initModule() {
    ...
    // Register callbacks
    BetterSMS::Game::registerBootCallback("BetterDebugEntry_autoDebug", autoDebug);
    BetterSMS::Stage::registerUpdateCallback("BetterDebugEntry_removeSetting", removeSetting);

    // Register settings
    sSettingsGroup.addSetting(&sRememberDebugSetting);
...
// Definition block
KURIBO_MODULE_BEGIN("BetterDebugEntry", "Angry_Max", "v1.0") {
